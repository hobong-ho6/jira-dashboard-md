...
- **⚠️ 코멘트 본문의 언더스코어(`_`)가 먹힌다 — 게시 전에 처리해야 한다(2026-08-06 실측).**
  ...
  - **URL 안의 `_` 는 손상되지 않았다(2026-08-06, UNIFY-9127·9128).** 요약 코멘트 말미의 원본 Slack
    링크에 든 `thread_ts=1784858045.050549` 는 백틱 없이 게시했는데도 그대로 저장됐다. 손상된
    `mini_luckyball_congrats_won` 은 독립 토큰이었다 — 즉 emphasis 오인은 `_` 가 **단어 경계에 노출된**
    경우에 일어나고, 다른 문자에 둘러싸인 URL 내부에서는 일어나지 않는 것으로 보인다(관측 1건에서
    추론한 것이므로 단정하지 않는다). 위 백틱 우회의 검증은 **독립 토큰형 `_` 가 든 코멘트를 게시할 때**
    해야 한다.
  - **⚠️ 손상 여부는 `jira_add_comment` 의 응답으로 판단하지 말고 재조회해서 확인한다.** 같은 실측에서
    add_comment 의 즉시 응답 JSON 은 본문을 이스케이프된 형태(`\*`, `thread\_ts`)로 돌려줬지만,
    `jira_get_issue(fields="comment")` 로 다시 읽은 실제 저장값은 원문 그대로였다. 즉시 응답 표기는
    저장본과 다를 수 있으므로, 손상 판단·검증은 항상 재조회 결과로 한다.
