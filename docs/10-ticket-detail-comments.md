...
- 하위 작업 추가: 제목 입력 + 추가 → 이 티켓을 부모로 `create_issue`(`issueType="Sub-task"`, `parent`=현재 키, `project`=키 접두사) 큐잉(`11`). **현재 이슈가 Sub-task면 숨긴다**(Sub-task는 하위를 못 가짐).
- 코멘트 영역(아래).
- 액션: 상태 변경, Due 변경(직접 입력 적용 + **+3일/+5일 버튼** — 설정된 마감일(입력값→저장값→오늘 순) 기준 N일 뒤로 즉시 `set_duedate` 큐잉 + **제거 버튼** — `duedate=null` 로 `set_duedate` 큐잉, 마감일이 있을 때만 표시), 코멘트 추가, 연결 추가, 하위 작업 추가(모두 `11`로 큐잉).
