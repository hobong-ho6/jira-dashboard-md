...
3. **서비스 헬스체크** (`docs/13` "프로세스 수명"):
   ...
   - `pgrep -f tools/watch_queue.py`로 워쳐 확인. 없으면 `python3 tools/watch_queue.py`를 `run_in_background`로 재기동한다(대기는 메인 세션 소유 — `docs/13`). 이 스킬이 서브 에이전트 안에서 실행 중이면 백그라운드 워쳐를 띄우지 말고(완료 알림이 종료된 자신에게 가 루프가 끊긴다), 최종 보고에 "워쳐 DOWN — 메인 세션에서 재기동 필요"를 명시한다.
   - 워쳐가 즉시 종료되며 pending 큐를 반환하면, MCP 처리는 `queue-worker` 서브 에이전트에 넘기고 반환된 payload 경로로 `tools/apply_and_rewatch.sh`를 실행한다(`docs/13` "한 배치 처리 흐름"). 무한 방치 금지.
