...
1. `HANDOFF.md` 를 **가장 먼저** 읽는다.
2. `git status && git log --oneline -5` 로 실제 상태를 확인하고, `HANDOFF.md` 의
   `마지막 커밋` 과 다르면 그 불일치를 먼저 사용자에게 보고한다.
3. `git pull --rebase` (uncommitted 변경이 있으면 pull 전에 사용자에게 알린다).
4. 한 줄 브리핑 후 착수 확인을 받는다.

절차 상세는 `.claude/skills/handoff/SKILL.md` 를 따른다.
사용자가 "핸드오프 갱신" / "세션 정리" / "handoff" 라고 하면 그 스킬의 세션 종료 절차를 실행한다.

## 이 저장소의 원칙

- 로컬에만 있는 것은 **없는 것**이다. 다른 PC로 넘겨야 할 상태는 항상
  `HANDOFF.md` + Git 커밋으로 남긴다. `git stash` 는 인수인계 수단이 아니다.
- 프로젝트 전용 스킬은 `.claude/skills/` 에 커밋한다. 개인 프로필에만 설치된
  스킬에 의존하지 않는다.
- 비밀값(토큰, API 키, `.env`)은 어떤 문서에도 기록하지 않는다.
  키 **이름**과 획득 방법만 `HANDOFF.md` 의 환경 섹션에 남긴다.
- Claude는 토큰·비밀번호를 채팅으로 요청하거나 대신 입력하지 않는다.
  인증이 필요하면 사용자가 직접 `gh auth login` / SSH 키 등록을 하도록 안내한다.
---
...
- "서버 켜" / "serve" / "워쳐 실행" → 서버 데몬 보장(`tools/ensure_services.sh`, 상시 데몬·세션 정리 생존) **+ `python3 tools/watch_queue.py`를 `run_in_background`로 기동**(항상 함께, 이미 떠 있으면 유지). **대기는 항상 메인 세션**이 하고, MCP 처리는 **무거운 배치만** `queue-worker` 서브 에이전트(`.claude/agents/queue-worker.md`)에 넘긴다 — 위임 기준은 `docs/13` "위임 판단" 표(slackUrl 요약·subtasks·sync 전체 재조회·3건 이상·개발 중일 때 위임, 읽기 전용과 단순 변경 1~2건은 메인 직접, 애매하면 직접이 기본). 워커 기동 고정비가 ~40~50k 토큰이라 가벼운 배치를 위임하면 손해다(실측). 어느 쪽이든 payload 경로로 `tools/apply_and_rewatch.sh`를 실행한다. 워쳐를 서브 에이전트에 맡기지 않는다(무한 블로킹 + 10분 상한으로 루프 끊김, 2026-07-30 실측). 세션 살아있을 때만 자동 처리 — 상세 `docs/13` "프로세스 수명"
